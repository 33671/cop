...
int main(int argc, char *argv[]) {
    ...
    const char *model   = app.models[0]->model_id;
    const char *api_key = app.models[0]->api_key;
    char api_endpoint[512];
    snprintf(api_endpoint, sizeof(api_endpoint),
             "%s/chat/completions", app.models[0]->base_url);
    ...
    if (app.models[0]->max_tokens > 0)
        llm_runtime_set_max_tokens(rt, app.models[0]->max_tokens);
