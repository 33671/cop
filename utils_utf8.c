...
/* 判断是否为全角/宽字符（East Asian Wide/Fullwidth + emoji）
   数据来自 isocline 的 mk_is_wide_char，基于 Unicode EAW + emoji
   注意：表必须按 first 升序排列！ */
static int is_wide_char(uint32_t cp) {
    static const struct interval wide[] = {
        ...
        {0x2753, 0x2755},     // ❓❔❕
        {0x2757, 0x2757},     // ❗
        // {0x2764, 0x2764},   // ❤ has EAW=N (Neutral), NOT wide -- removed per isocline ref
        {0x2795, 0x2797},     // ➕➖➗
        {0x27B0, 0x27B0},     // ➰
        {0x27BF, 0x27BF},     // ➿
        {0x2B1B, 0x2B1C},     // ⬛⬜
        ...
        {0x2B55, 0x2B55},     // ⭕
        // CJK 及全角字符
        {0x2E80, 0x2FDF},     // CJK Radicals Suppl. + Kangxi Radicals
        {0x2FF0, 0x303E},     // IDC + CJK Symbols
        {0x3040, 0x3247},     // Hiragana..Enclosed CJK
        {0x3250, 0x4DBF},     // Enclosed CJK..CJK Ext A
        {0x4E00, 0xA4CF},     // CJK Unified..Yi Radicals
        ...
        {0x16FE0,0x16FE1},    // Tangut/Henian tick
        ...
        {0x1F9C1,0x1F9CF},    // 🧁~🧏
        {0x1F9D0,0x1F9FF},    // 🧐~🧿
        // 以下为较新 Unicode 版本补充的宽字符（EAW=W），isocline 原始表中未包含
        {0x1FA70,0x1FA7C},    // 🩰~🩼
        {0x1FA80,0x1FA88},    // 🪀~🪈
        {0x1FA90,0x1FABD},    // 🪐~🪽
        {0x1FABF,0x1FAC5},    // 🪿~🫅
    ...
    return bisearch(cp, wide, (int)(sizeof(wide)/sizeof(wide[0])) - 1);
}

/* 判断是否为零宽字符（combining marks / format chars 等）
   数据来自 isocline 的 mk_wcwidth 中的 combining 表 */
static int is_zero_width(uint32_t cp) {
    static const struct interval zw[] = {
    ...
    return bisearch(cp, zw, (int)(sizeof(zw)/sizeof(zw[0])) - 1);
}
...
int utf8_char_width(const char *s, int *bytes) {
    ...
    *bytes = len;
    ...
    // 控制字符（匹配 isocline mk_wcwidth 行为）
    if (codepoint == 0) return 0;                    // NUL
    if (codepoint < 0x20) return -1;                 // C0 控制字符 (01-1F)
    if (codepoint >= 0x7F && codepoint < 0xA0) return -1;  // DEL 及 C1 控制字符

    // 零宽字符（combining marks, format chars, 异体选择符等）
    if (is_zero_width(codepoint)) return 0;

    // 全角/宽字符
    if (is_wide_char(codepoint)) return 2;
